...
async function callNIM(nimModel, body) {
  ...
  const payload = {
    ...
    stream:      true, // всегда стримим от NIM
...
async function callNIMWithFallback(nimModel, body) {
  const chain = [nimModel, ...(FALLBACK_CHAIN[nimModel] || [])];
  let lastError;
  for (const model of chain) {
    try {
      console.log(`[NIM] Попытка: ${model}`);
      return { stream: await callNIM(model, body), usedModel: model };
    } catch (err) {
      lastError = err;
...
// Читает SSE-поток NIM и возвращает собранный текст
function collectStream(nimStream) {
  return new Promise((resolve, reject) => {
    let buf = '', content = '', reasoning = '', lastChoice = null, usage = null;
    ...
    nimStream.on('end',   () => resolve({ content, reasoning, lastChoice, usage }));
...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    // ─────────────────────────────────────────
    // stream: true → SSE (JanitorAI напрямую)
    // ─────────────────────────────────────────
    ...
    // ─────────────────────────────────────────
    // stream: false → обычный JSON (LoreBary)
    // ─────────────────────────────────────────
    } else {
      const { content, reasoning, lastChoice, usage } = await collectStream(nimStream);
      ...
      res.json({
        ...
        choices: [{
          index:        0,
          message:      { role: 'assistant', content: mergeContent(content, reasoning) },
...
app.listen(PORT, () => {
  ...
  console.log(`   stream:true  →    SSE  (JanitorAI)`);
  console.log(`   stream:false →    JSON (LoreBary)\n`);
});
