...
// Универсальный парсер — JSON при любом Content-Type
app.use((req, res, next) => {
...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    const { stream: nimStream } = await callNIMWithFallback(nimModel, body);

    // ✅ ВСЕГДА отвечаем SSE — независимо от stream: true/false в запросе.
    // LoreBary раньше работала со стримингом, значит её клиент умеет читать SSE.
    // Это единственный способ не упасть по таймауту при reasoning:max.
    ...
    // Keepalive каждые 15 сек — держит соединение для Render и LoreBary
...
app.listen(PORT, () => {
  ...
  console.log(`   Ответ клиенту:    всегда SSE (stream:true и stream:false)\n`);
});
