  ...
  function tryOnce() {
    return fn(attempt).catch(function(err) {
      ...
      if (status === 429) {
        ...
        delay = ra > 0 ? ra * 1000 : 30000; // 30s fixed if no header
      } else {
...
app.get('/health', function(_req, res) {
  res.json({
    ...
    queue:             { active: activeRequests, waiting: requestQueue.length, max_concurrent: MAX_CONCURRENT }
  });
});
...
app.post('/v1/chat/completions', requireApiKey, function(req, res) {
  ...
  // Always open as SSE immediately — prevents Render/Railway 30s request timeout
  // from killing the connection while NIM cold-starts.
  res.setHeader('Content-Type',  'text/event-stream');
  res.setHeader('Cache-Control', 'no-cache');
  res.setHeader('Connection',    'keep-alive');
  res.flushHeaders();

  // Send a heartbeat comment every 5s so the connection stays alive.
  ...
  // If client disconnects early, stop everything.
  ...
  .then(function(upstream) {
    ...
    upstream.data.on('end', function() {
      ...
      if (clientWantsStream) {
        ...
        // Non-stream client: emit a single synthetic SSE event with the full JSON,
        // then close. The client receives it as a regular fetch response body.
  ...
  .catch(function(err) {
    ...
    var isTimeout = err.code === 'ECONNABORTED' || err.code === 'ETIMEDOUT' || err.code === 'STREAM_TIMEOUT' || status === 504;
    var message   = isTimeout
      ? 'Model ' + nimBody.model + ' did not respond in time after retries. Please try again.'
      : ((detail.nim_error && (detail.nim_error.detail || (detail.nim_error.error && detail.nim_error.error.message))) || err.message || 'Proxy error');

    // Send error as SSE event (headers already sent)
    var errPayload = { error: { message: message, type: isTimeout ? 'timeout_error' : 'api_error', code: status || 500, model: nimBody.model } };
