...
// DeepSeek V4 — 3 режима:
//   'low'  → Non-think  (быстро, но сухо)
//   'high' → Think High (баланс — РЕКОМЕНДУЕТСЯ для ролеплея)
//   'max'  → Think Max  (глубоко, НО первый токен может идти 2+ минуты —
//            многие клиенты (Janitor/LoreBary) сдаются раньше и кажут
//            "Failed to fetch", хотя сервер в итоге отвечает без ошибок)
const DEEPSEEK_REASONING_EFFORT = process.env.DEEPSEEK_REASONING_EFFORT || 'high';

const FIRST_TOKEN_TIMEOUT = parseInt(process.env.FIRST_TOKEN_TIMEOUT || '150000');
const IDLE_TIMEOUT        = parseInt(process.env.IDLE_TIMEOUT        || '60000');

// ✅ Как часто слать keepalive-чанк клиенту, пока NIM ещё думает (мс).
//    Держим короче типичных клиентских таймаутов (обычно 30-60с).
...
app.get('/health', (_, res) => {
  const mode = (DEEPSEEK_REASONING_EFFORT || 'high').toLowerCase();
  res.json({
    status: 'ok',
    service: 'OpenAI → NVIDIA NIM Proxy',
    nim_key_set: !!NIM_API_KEY,
    deepseek_mode: mode,
    deepseek_mode_warning: mode === 'max'
      ? '⚠️ Think Max может идти 1-2+ минуты до первого токена — многие клиенты (Janitor/LoreBary) сдаются раньше по своему таймауту и показывают "Failed to fetch", хотя сервер потом всё равно отвечает без ошибок. Рекомендуется "high".'
      : 'ok',
...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    if (clientWantsStream) {
      ...
      // ✅ ФИКС: раньше слали SSE-комментарий (": keepalive\n\n") — по спеке
      //    он невидим, но клиенты с самодельным SSE-парсером (не строгий
      //    EventSource) могут его не распознавать как "активность" и всё
      //    равно посчитать соединение мёртвым. Теперь шлём ВАЛИДНЫЙ чанк
      //    в формате OpenAI (пустая delta) — это выглядит как настоящий
      //    "думаю…" сигнал для любого OpenAI-совместимого клиента.
...
app.listen(PORT, () => {
  ...
  if ((DEEPSEEK_REASONING_EFFORT || '').toLowerCase() === 'max') {
    console.log(`   ⚠️  'max' часто вызывает Failed to fetch на стороне клиента (первый токен 1-2+ мин)`);
  }
  console.log(`   RP style prompt:   ${ENABLE_RP_STYLE_PROMPT}`);
  console.log(`   Keepalive every:   ${KEEPALIVE_INTERVAL / 1000}s (валидный чанк, не SSE-комментарий)`);
